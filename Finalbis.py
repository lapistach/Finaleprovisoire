...
class Monomers:
    ...
    def assignRadiaiMassesVelocities(self, NumberMono_per_kind = np.array([4]), Radiai_per_kind = 0.5*np.ones(1), Densities_per_kind = np.ones(1), k_BT = 1 ):
        ...
        for i in range(self.NM):
            ...
            vitesse = np.sqrt( k_BT *2 / self.mass[i])
            self.vel[i] *= vitesse
    ...
    def assignRandomMonoPos(self, start_index = 0 ):
        ...
        while self.mass[new_mono]<1000: #looking for the seed particle so that we can place it first
              new_mono +=1
              a = new_mono #keeping the indice of the seed particle in memory
        self.pos[new_mono,:]= [(L_xMax-L_xMin)/2 ,(L_yMax-L_yMin)/2 ] #placing the seed particle
        if new_mono == 0: #if the seed particle was at indice 0 then place the other particles starting at indice 1
           new_mono += 1
        else:
            new_mono = 0
        while new_mono < self.NM and infiniteLoopTest < 10**4:
            if new_mono == a and (new_mono + 1) < self.NM: #verifying if we are on the seed particle or not
               new_mono += 1 #if yes, then we look at the next particle
            elif new_mono == a and (new_mono + 1) > self.NM: #verifying if the next particle really exists
                infiniteLoopTest = 10**5 #if it doesn't exist then we're finished
            else:  #if we're not on the seed particle AND the particle exists, then we can place it randomly
    ...
    def Mono_pair_time(self):
        ...
        deltat = np.where(condition ,(-b-np.sqrt(delta))/(2*a),np.array([np.inf]*len(condition)))
    ...
    def compute_new_velocities(self, next_event):
        ...
        if next_event.Type == 'wall' :
            ...
            if self.NM>2 : 
                if (self.mass[mono_1]>1000) or (self.mass[mono_2]>1000) :
